...
from metrics import cassandra_read_duration_seconds, cassandra_write_duration_seconds, cassandra_connected_hosts
...
class CassandraClient:
    ...
    def update_host_metrics(self):
        if self.cluster and self.cluster.metadata:
            cassandra_connected_hosts.set(len(self.cluster.metadata.all_hosts()))
    ...
    def _timed_read(self, stmt, params, **kwargs):
        start = time.time()
        result = self.session.execute(stmt, params, **kwargs)
        cassandra_read_duration_seconds.observe(time.time() - start)
        return result

    def is_processed(self, event_id: str) -> bool:
        row = self._timed_read(
            self._stmts["check_processed"], [event_id], execution_profile="quorum"
        ).one()
        return row is not None

    def get_zone_inventory(self, product_id: str, zone_id: str) -> dict:
        row = self._timed_read(self._stmts["get_pz_inv"], [product_id, zone_id]).one()
        if row:
            return {
    ...
    def get_product_inventory(self, product_id: str) -> dict:
        row = self._timed_read(self._stmts["get_prod_inv"], [product_id]).one()
        if row:
            return {"total_available": row.total_available or 0, "total_reserved": row.total_reserved or 0}
        return {"total_available": 0, "total_reserved": 0}

    def get_zone_product_inventory(self, zone_id: str, product_id: str) -> dict:
        row = self._timed_read(self._stmts["get_zone_inv"], [zone_id, product_id]).one()
        if row:
            return {"available": row.available_quantity or 0, "reserved": row.reserved_quantity or 0}
        return {"available": 0, "reserved": 0}

    def get_order(self, order_id: str):
        return self._timed_read(self._stmts["get_order"], [order_id]).one()
    ...
    def _timed_batch(self, batch):
        start = time.time()
        self.session.execute(batch)
        cassandra_write_duration_seconds.observe(time.time() - start)

    def execute_inventory_update(
        self, event_id, event_type, product_id, zone_id,
        new_avail, new_reserved, event_ts,
        prod_total_avail, prod_total_reserved,
        zone_avail, zone_reserved, history_qty, supplier_id=None,
    ):
        batch = self._new_batch()
        now = self._now()
        batch.add(self._stmts["upsert_pz"],
                  [product_id, zone_id, new_avail, new_reserved, event_id, event_ts, now, supplier_id])
        batch.add(self._stmts["upsert_prod"],
                  [product_id, prod_total_avail, prod_total_reserved, now])
        batch.add(self._stmts["upsert_zone"],
                  [zone_id, product_id, zone_avail, zone_reserved, now])
        batch.add(self._stmts["insert_processed"],
                  [event_id, event_type, now])
        batch.add(self._stmts["insert_history"],
    ...
    def execute_move_update(
        self, event_id, event_type, product_id,
